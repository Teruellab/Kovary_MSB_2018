function [t,x, simdata, x_end] = erkSimulate(stimulus, names, params_mod, init_mod, options)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
% erkSimulate(stimulus, names, params_mod, init_mod, options)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
%
% INPUTS:
% stimulus     (n x 2) cell matrix with stimulus name and value pairs (e.g. {'RasGTP',0.1})
...
% init_mod     (n x 2) cell matrix with species name and initial value pairs (e.g. {'ERK', 0.1})
...
v.DT = 1;
v.SIM_TIME     = 180*60; %  length of stimulation phase (phase 2), in sec
...
[v.PARAMS, v.SPECIES] = erkInitialize();
...
% Set steady-state concentrations
v.SPECIES.INIT  = zeros(length(v.SPECIES.NAMES),1); % all species start at zero by default
v.SPECIES.INIT(strcmp('RasGTP',v.SPECIES.NAMES))   = 0;
v.SPECIES.INIT(strcmp('Raf',v.SPECIES.NAMES))   = 40000;    

v.SPECIES.INIT(strcmp('Phase1',v.SPECIES.NAMES))   = 40000;    
v.SPECIES.INIT(strcmp('MEK',v.SPECIES.NAMES))   = 21000000;    
v.SPECIES.INIT(strcmp('Phase2',v.SPECIES.NAMES))   = 400000;    

v.SPECIES.INIT(strcmp('ERK',v.SPECIES.NAMES))   = 22100000;    
v.SPECIES.INIT(strcmp('Phase3',v.SPECIES.NAMES))   = 10000000; 
v.SPECIES.INIT(strcmp('Phase4',v.SPECIES.NAMES))   = 40000; 
...
if ~isfield(v,'STEADY_STATE')
    v.PHASE     = 1;
    [~, r1] = ode15s('erkOde', [v.T_EQUILIBRATE 0], v.SPECIES.INIT,ode_opt,v);
    ...
    iter = 0;
    while ~converge
        [~, r1] = ode15s('erkOde', [v.T_EQUILIBRATE 0], starting_vals,ode_opt,v);
...
v.PHASE = 2;
erkOde([],[],[],v);
[t_sim, x_sim] = ode15s('erkOde', [0 v.SIM_TIME],starting_vals,ode_opt,v);
